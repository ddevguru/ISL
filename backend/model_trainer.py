...
def train_sign_model():
    ...
    try:
        ...
        label_encoder.fit(list(range(len(sign_labels))))
        ...
        for sign_idx, (sign_name, trans) in enumerate(sign_labels.items()):
            ...
            for _ in range(samples_per_sign):
                ...
                for _ in range(sequence_length):
                    ...
                    keypoints = np.random.randn(1662).astype(np.float32)
        ...
        model = Sequential([
            Bidirectional(LSTM(128, return_sequences=True), input_shape=(sequence_length, 1662)),
            Dropout(0.2),
